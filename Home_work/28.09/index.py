        ...
        if request.method == "POST":
            if post == "login":
                ...
                if new_login == "" or new_password == "":
                    return render_template('login.html', error="Login or password not entered")
                elif new_login == "":
                    return render_template('login.html', error="Password not entered")
                elif new_password == "":
                    return render_template('login.html', error="Password not entered")
                else:
                ...
                if log == "" or password == "":
                    return render_template('login.html', error="Login or password entered incorrectly")
                else:
                    data_users = return_db_user()
                    for i in data_users:
                        if i[1] == log and i[2] == password:
                            ...
                            return redirect(url_for('.home', id_user=id_user, login_user=login_user))
                    return render_template('login.html', error="This user doesn't exist")
